...
if (isset($_SESSION["ud"])) {
    ...
    if ($adress_valid_num > 0) {
        ...
        $newqty = 0;
        if ($cart_data["count"] === null) {
            $newqty = $singleqty;
        } else {
            $newqty = $cart_data["count"];
        }
        ...
        Database::iud("INSERT INTO `buyitems`(`date`,`count`,`product_id`,`user_email`,`buy_price`,`order_id`) 
            VALUES ('" . $date . "','" . $newqty . "','" . $pid . "' , '" . $user_email . "' , '" . $amount . "' , '" . $order_id . "')");
    } else {
        echo "Address Error";
    }
} else {
